...
/** Hand returns back after loading ascorbic */
function handBack(scope) {
    getChild("hand_with_glass").visible = false; /** Hand with glass image set visible false */
    getChild("hand").visible = true; /** Hand and ascorbic image set visible true */
    getChild("ascorbic").visible = true;
    scope.graph_disable = false; /** Enable Plot Graph button */
    var hand_return_tween = createjs.Tween.get(getChild("hand")).to({
        x: 350, y: 700
    }, 1000); /** Tween of hand movement after loading ascorbic */
    scope.$apply();  
}

/** Load Ascorbic butten event */
function loadAscorbicEvent(scope) {
    var hand_movement_tween = createjs.Tween.get(getChild("hand_with_glass")).to({
        x: 60, y: 550
    }, 1000); /** Hand tween movement for loading ascorbic */
    setTimeout(function() { handBack(scope)}, 1500); /** Hand returns back after loading ascorbic */
...
function plotGraphEvent(scope) {
    ...
    scope.load_ascorbic_disable = true; /** Load Ascorbic Acid button disabled */
}

/** Initial mV and final mV text value validation */
function inputValidation(scope, dialogs) {
    if ( scope.mVinitial > -150 ) { /** If initial mV greater than -300 */
        dialogs.error(); /** Error alert */
        scope.mVinitial = -150; /** Set default initial mV */
    } else if ( scope.mVfinal > -300 ) { /** If final mV greater than -608 */
        dialogs.error(); /** Error alert */
        scope.mVfinal = -500; /** Set default final mV */
    }
}
...
function calculation(scope) {
    ...
    if ( scope.scan_analysis == 1 ) { /** If the graph is polarogram */
        for ( var i=0.1; i<=LOOP_COUNT_CONST; i+=.1 ) {
        ...
        for ( var j = y_axis.length; j<=LOOP_COUNT_CONST; j++ ) {        
        ...
        /** The other six values are same, that is the x_axis_array_fourth value */ 
        x_axis_array.push(x_axis_array_fourth, x_axis_array_fourth, x_axis_array_fourth, x_axis_array_fourth, x_axis_array_fourth, x_axis_array_fourth);
        ...
        /** Push first two values to x_axis_array_value */
        x_axis_array_value.push(0, scope.mVinitial); 

        for ( var m=2; m<=LOOP_COUNT_CONST; m++ ) {
            /** Calculating the rest of the values for x_axis_array_value */
            var x_axis_array_value_rest = slope*y_axis[m]-MIN_FINAL_MV_CONST; 
            x_axis_array_value.push(x_axis_array_value_rest); /** Push the values to x_axis_array_value */
        }
        for ( var n=0; n<=LOOP_COUNT_CONST; n++ ) { 
        ...
        for ( var i=0.1; i<=LOOP_COUNT_CONST; i+=0.1 ) {
            y_axis_array.push(i); /** Rest of the values are pushed in y_axis_array */
            if ( i < center ) {
        ...
        for ( var k = y_axis.length-1; k<=LOOP_COUNT_CONST; k++ ) {        
        ...
        for ( var n=2; n<=LOOP_COUNT_CONST; n++ ) {
            /** Calculating the rest of the values for x_axis_array_value */
            var x_axis_array_value_rest = (parseFloat(slope)*parseFloat(y_axis_array[n]))-MIN_FINAL_MV_CONST;
            x_axis_array_value.push(x_axis_array_value_rest); /** Push the values to x_axis_array_value */
        }
        for ( var p=0; p<=LOOP_COUNT_CONST; p++ ) {
            /** Check whether the y_axis_array value is less than or equal to double of center value */
            if ( y_axis_array[p] < (average_diffusion_half-0.1) ) {
                x_axis.push(x_axis_array_value[p]); /** Push the x_axis_array_value to x_axis */
            }
        } 
        for ( var q = x_axis.length; q<=LOOP_COUNT_CONST; q++ ) {            
            x_axis.push(scope.mVfinal); /** Set rest of the value as mv final value in x_axis array */        
        }
        for ( var r = 0; r<=LOOP_COUNT_CONST; r++ ) {  
